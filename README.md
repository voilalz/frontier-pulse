...
- **每日深读**：新栏目从过去 24 小时的独立事件中选取默认 12 项（可配置 10–15 项），生成导语、主题章节、逐事件分析和结语。来源链接和图片只由原始新闻元数据附加；图片缺失或加载失败时正常显示正文。长文结构不合格时逐章节恢复写作，文章或章节不完整时明确标记并保留基于已有摘要的事实编排，不用旧闻凑数。事件编号、原始摘要和引用始终由程序固定；结构校验不等同于逐句事实核验。少于 10 件时明确呈现实际数量。
...
`public/assets/news-policy.json` 是采集器与网页共用的选题规则：排除以中国、中国机构或企业为主要对象的新闻。采用标题和导语中的中英文实体匹配，不按媒体注册地或作者国籍判断；后文顺带提及中国不自动排除。它是可维护的编辑规则，不能识别所有未列出的实体或隐含指代。旧归档、离线缓存、收藏、关注、搜索与论文里的新闻链接也应用此规则；论文条目本身不按作者国籍过滤。

新增并核验 13 个 RSS/Atom 入口：JAXA、Naval News、DroneLife、sUAS News、The Robot Report、MIT News AI、Google Research、NVIDIA Blog、Phys.org Technology、ScienceDaily Engineering、France 24、Deutsche Welle、The Guardian World。信源配置数从 18 扩充至 31，实际每日可用条数仍取决于各站更新及可访问性；保持来源/主题配额和同事件去重。

日报/状态 schema 升级至 10，动态流升级至 6。工作流变更合并后自动重新生成当前日报；这类代码发布补跑不再重复发送管理员邮件。正常定时邮件流程保留。

以下事件情报与评分章节描述内部数据能力，不代表这些字段会在新闻卡片中显示。
...
- 每条 Top 10 自动回看近 365 日归档，新闻卡片下的“事件时间线与分析”默认折叠，展开后按归档日期展示历史报道、本次进展、脉络分析与条件性后续观察。历史标题可跳转当期报道；关联评分保留在内部，历史节点沿用当前选题过滤。规则先锁定历史证据，AI 只能总结给定候选，不能新增或替换历史记录。
...
- 新闻与论文使用标题、摘要、关键词和领域进行双向关联；关联保留在数据层，论文卡可查看符合选题规则的近期现实动态。
- 每个事件附带“证据/争议矩阵”与可验证的预判台账：记录独立来源组、否认/争议线索、到期日、验证信号和开放/到期状态，不把条件性判断伪装成确定结论。
- 展示新闻原始来源；来源独立性与评分分项仅保留在后台数据中。
- 按日归档、日期前后切换，以及按月分片的轻量跨日期搜索；更多来源在展开时按需读取当期归档。
