...
RSS 同时提供导语和 `content:encoded` / Atom 正文时，采集器选取更完整的文本（最多 6,000 字符）。日报和动态摘要以 180–320 个汉字为目标，概括事件、背景、关键细节、进展和后续安排；来源不足时允许更短，不补造信息。仅有标题或短导语时，无法承诺覆盖完整报道。旧的短摘要缓存按摘要修订号及输入文本指纹失效；未变化的新摘要继续复用。
...
- 每条 Top 10 自动回看近 365 日归档，通过标题、摘要、关键事实和标签计算可解释的关联度，在内部保存关联事件时间线、阶段演化总结与条件性后续观察；规则先锁定历史证据，AI 只能总结给定候选，不能新增或替换历史记录。
